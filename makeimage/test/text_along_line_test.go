	...
	for name, tt := range tests {
		for zoom := 1; zoom <= 10; zoom++ {
			...
			if tt.showOutlines {
				...
				err = text.DrawGlyphOutlines(gc, tt.text, scaledGeom, typeFace)
				if err != nil {
					t.Fatal(err)
				}
			}
			...
			// text along line
			glyphs, err := text.TextAlongLine(gc, tt.text, scaledGeom, typeFace)
			if err != nil {
				t.Fatal(err)
			}
...
func TestTurfHillRoadRochdale(t *testing.T) {
	...
	for name, tt := range tests {
		...
		// text along line
		glyphs, _ := text.TextAlongLine(gc, tt.text, scaledGeom, typeFace)
		for _, glyph := range glyphs {
			err = geom.DrawRune(gc, glyph.Pos, face, glyph.Rotation, glyph.Char)
			if err != nil {
				t.Fatal(err)
